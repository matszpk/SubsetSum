...
__kernel void subsetSumHashKernel(uint n, uint groupVal, uint groupBits, uint hlIndex,
           const constant long* sumChanges, uint hashBits,
                                 ...
                                 )
{
    ...
    const uint groupMask = hmask-((1U<<(hashBits-groupBits))-1U);
    const uint ghentryMask = hmask&(~groupMask);
    const uint ngroupMask = hmask-((1U<<(hashBits-groupBits-2))-1U);
    
    const long inputSum = inputSums[gi];
    
    uint curFoundIdx = 0xffffffffU;
    
    for (uint groupId = 0; groupId < 4; groupId++)
    {
        const uint ngroupVal = groupVal|(groupId<<(hashBits-2-groupBits));
        for (uint current = 0; current < 256; current++)
        {
            ...
            uint hkey = (sum^temph1^temph2^temph3^temph4)&hmask;
            bool found = false;
            if ((hkey & ngroupMask) != ngroupVal)
                continue;
            hkey &= ghentryMask;
            if (hashTable[hkey].size != 0)
            {
                ...
                const global uint* hsubsets = hashSubsets +
                        upsample(hashTable[hkey].poshi,hashTable[hkey].poslo) - hlIndex;
